...
def read_csv_drop_cols(filepath, csv_filename: str, list_cols: list, drop: bool):
    """Read a CSV file and either drop or keep specified columns.
    ...
    list_cols : list
        A list of strings representing the names of the columns to drop or keep.
    drop : bool
        If True, drop the specified columns. If False, keep only the specified columns.
    
    Returns
    -------
    df : pd.DataFrame
        A DataFrame containing the data from the CSV file with the specified columns either dropped or kept.
    """
    df = pd.read_csv(filepath + csv_filename, skipinitialspace=True)
    if drop:
...
def remove_cols_that_startwith(df: pd.DataFrame, list: list):
    """Remove columns from a DataFrame that start with a specified string.
    ...
    list : list
        A list of strings that represent the start of the column names to remove.
    ...
    """
    for col in list:
        df.drop([c for c in df.columns if c.startswith(col)], axis=1, inplace=True)
    return df
...
def create_cohort_df(xlsx : pd.ExcelFile, sheet : str) :
    ...
    [cohort_df[item] = '' for item in ['Enrollment.Subtype', 'Consensus.Subtype']]
...
def decode_columns(df : pd.DataFrame, cols : List, two_option = 'None', decode_option : str):
    
    """Decodes all columns in the list in a dataframe based on the decode_option string.

    Args: 
        df: a Pandas DataFrame
        cols: a list of column names
        two_option (optional): a string to be used as the replacement value for the integer 2 when decode_option is 'yes/no'
        decode_option (optional): a string indicating how the values in cols should be decoded. Acceptable values are 'yes/no' or 'none2severe'.
        
    Returns: 
        df: a modified version of the input DataFrame with the values in the specified columns decoded according to the decode_option

    """
    
    if decode_option == 'yes/no':
        for col in cols:
            df[col] = df[col].astype(int, errors = "ignore")
            df[col].replace({0 : 'No' , 1: 'Yes' , 2 : two_option}, inplace = True)
    elif decode_option == 'none2severe':
        for col in cols:
            df[col] = df[col].astype(int, errors = "ignore")
            df[col].replace({0 : 'None', 1 : 'Slight', 2: 'Mild', 3 : 'Moderate', 4 : 'Severe'}, inplace = True)
    else:
        print('Please enter a valid decode_option')
...
def merge_dti_results(ppmi_merge, bucket, prefix, search_string, merge_on):
    """Merge in FA and MD DTI results 

    Args:
        ppmi_merge (df): _description_
        bucket (str): _description_
        prefix (str): _description_
        search_string (str): _description_
        merge_on (str): _description_
    
    Return :
        df
    """
    def read_s3_csv(key):
        csv_obj = boto3.client('s3', region_name="us-east-1").get_object(Bucket=bucket, Key=key)
        body = csv_obj['Body']
        csv_string = body.read().decode('utf-8')
        df = pd.read_csv(StringIO(csv_string))
        df.drop(['u_hier_id','u_hier_id.1'], axis=1, inplace=True)
        df['Subject.ID'] = np.int64(key.split('/')[4])
        df['Event.ID.Date'] = key.split('/')[5][4:6] + '/' + key.split('/')[5][0:4]
        df['DTI.antspymm.Image.ID'] = key.split('/')[7]
        if 'md' in search_string: # FIXME- if you fix md labels you can get rid of this 
            df.columns = df.columns.str.replace("fa", "md")
        return df

    print(f"Merge in {search_string}")
    appended_data_dti = pd.concat([read_s3_csv(key) for key in search_s3(bucket, prefix, search_string)])
    return pd.merge(ppmi_merge, appended_data_dti, on=merge_on, how="outer")
...
def fill_subtype(df : pd.DataFrame) :
    if df['Cohort'][0] == 'Parkinson\'s Disease':
        # Decode Enrollment.Subtype
        df.loc[(df['ENRLPD'] == 1) & (df['ENRLLRRK2'] == 1), 'Enrollment.Subtype'] =  ' : LRRK2'
        df.loc[(df['ENRLPD'] == 1) & (df['ENRLGBA'] == 1), 'Enrollment.Subtype'] = ' : GBA'
        df.loc[(df['ENRLPD'] == 1) & (df['ENRLSNCA'] == 1), 'Enrollment.Subtype'] =  ' : SNCA'
        # Decode Consensus.Subtype
        df.loc[(df['CONPD'] == 1) & (df['CONLRRK2']== 1) & (df['CONGBA'] == 0) & (df['CONSNCA'] == 0), 'Consensus.Subtype'] = 'Genetic : LRRK2'
        df.loc[(df['CONPD'] == 1) & (df['CONLRRK2'] == 0) & (df['CONGBA'] == 1) & (df['CONSNCA'] == 0) , 'Consensus.Subtype'] = 'Genetic : GBA'
        df.loc[(df['CONPD'] == 1) & (df['CONLRRK2'] == 1) & (df['CONGBA'] == 1) & (df['CONSNCA'] == 0), 'Consensus.Subtype'] = 'Genetic : LRRK2 + GBA'
        df.loc[(df['CONPD'] == 1) & (df['CONLRRK2'] == 0) & (df['CONGBA'] == 0) & (df['CONSNCA'] == 1) , 'Consensus.Subtype'] = 'Genetic : SNCA'
...
def add_PD_Disease_Duration(df: pd.DataFrame, col_name: str):
    ...
    for diag_date, event_date in zip(df['PD.Diagnosis.Date'], df['INFODT']):
        if isinstance(diag_date, str) and isinstance(event_date, str):  # If we have both a PD Diagnosis date and an event id date
            diag_year, diag_month = map(int, diag_date.split('/'))
            event_year, event_month = map(int, event_date.split('/'))
            diff = relativedelta.relativedelta(datetime(event_year, event_month, 1), datetime(diag_year, diag_month, 1))
            df[col_name] = ((diff.years) * 12 + diff.months) / 12  # PD.Diagnosis.Duration in years
    return df
...
def add_LEDD(ppmi_merge, ppmi_download_path) : 
    ...
    LEDD_med_df = read_csv_drop_cols(ppmi_download_path, 'LEDD_Concomitant_Medication_Log.csv', ['PATNO','LEDTRT', 'LEDD', 'STARTDT', 'STOPDT'], drop = False)
...
def make_categorical_LEDD_col(df, df_col, new_df_col, num_categories):
    ...
    """This function takes in a DataFrame, a column in the DataFrame, a new column name, and a number of categories. 
    It creates a new column in the DataFrame with the specified name and assigns categorical values to it based on the 
    values in the input column, dividing the values into the specified number of categories.

    Args : 
        df: a Pandas DataFrame
        df_col: a column in df
        new_df_col: a string representing the name of a new column to be created in df
        num_categories: an integer representing the number of categories to divide df_col into

    Returns:
        df : a modified version of df with a new column called new_df_col that contains categorical values based on the values in df_col

    """
    
    df_col_2_list = sorted(set(df[df_col].dropna().to_list())) # Get a sorted list of all unique values in df_col
    lists = np.array_split(df_col_2_list, num_categories) # Split the list into num_categories equal parts
    
    # Iterate through the lists and assign values to new_df_col
    for i, lst in enumerate(lists):
        df.loc[(df[df_col] >= lst[0]) & (df[df_col] <= lst[-1]), new_df_col] = f'OnQ{i+1}'
    return df
...
def format_genetics_df(genetics_file_path : str, csv_filename) :
    ...
    Args:
        genetics_file_path (str): _description_
        csv_filename (_type_): _description_

    Returns:
        genetics_df: reformatted df 
    """
    ...
    genetics_df['Subject.ID'] = genetics_df['Subject.ID'].apply(lambda x: x.split('_')[-1])
    return genetics_df
...
def add_datiq(ppmi_merge, datiq_path) : 
    ...
    for df in dfs:
        ...
        df['PATNO'], df['INFODT'] = zip(*df['subjNames'].apply(lambda x: x.split('_')))
        df['INFODT'] = df['INFODT'].apply(lambda x: x.split('-')[0])
        df.drop('subjNames', axis=1, inplace=True)
    ...
    prodromal_info['INFODT'] = prodromal_info['ScanDate'].apply(lambda x: datetime.strptime(x,'%d-%b-%y').date() )        
    prodromal_info['INFODT'] = prodromal_info['INFODT'].astype(str).str.replace('-','') # Reformat date 
    ...
    prodromal_datiq['PATNO'] = prodromal_datiq['subjNames'].apply(lambda x: x.split('_')[2])
...
def add_mri_csv(ppmi_merge, ppmi_download_path) : 
    ...
    mri_df = decode_columns(mri_df, ['MRIWDTI','MRIWRSS','MRIRSSDF'], two_option = 'Uncertain', decode_option = 'yes/no')
...
def setup_updrs_df(updrs_filename, drop_cols, decode_dict, rename_col_dict) :
    ...
    updrs_df = read_csv_drop_cols(ppmi_download_path, updrs_filename,drop_cols, drop = True)
...
full_df = decode_columns(full_df, ['PHENOCNV'], two_option = 'Uncertain', decode_option = 'yes/no')
...
ppmi_merge = decode_columns(ppmi_merge, ['COMPLT'], two_option = 'Uncertain', decode_option = 'yes/no')
...
ppmi_merge = decode_columns(ppmi_merge, ['TRBUPCHR', 'WRTSMLR', 'VOICSFTR', 'POORBAL', 'FTSTUCK', 'LSSXPRSS', 'ARMLGSHK', 'TRBBUTTN', 'SHUFFLE', 'MVSLOW', 'TOLDPD'], two_option = 'Uncertain', decode_option = 'yes/no')
...
ppmi_merge = decode_columns(ppmi_merge, ['COGDECLN', 'FNCDTCOG'], two_option = 'Uncertain', decode_option = 'yes/no'))
...
ppmi_merge = merge_csv(ppmi_download_path, ppmi_merge, 'Benton_Judgement_of_Line_Orientation.csv', ['PATNO', 'EVENT_ID', 'JLO_TOTCALC'], merge_on = ['PATNO', 'EVENT_ID'], merge_how = "outer")
...
ppmi_merge = merge_csv(ppmi_download_path, ppmi_merge, 'Hopkins_Verbal_Learning_Test_-_Revised.csv',['PATNO', 'EVENT_ID', 'DVT_DELAYED_RECALL', 'DVT_TOTAL_RECALL'], merge_on = ['PATNO', 'EVENT_ID'], merge_how = "outer")
...
ppmi_merge = decode_columns(ppmi_merge, rem_list, two_option = 'Uncertain', decode_option = 'yes/no'))
...
updrs_part3_df_copy = decode_columns(updrs_part3_df_copy, ['NP3SPCH', 'NP3FACXP', 'NP3RIGN', 'NP3RIGRU', 'NP3RIGLU', 'NP3RIGRL', 'NP3RIGLL', 'NP3FTAPR', 'NP3FTAPL', 'NP3HMOVR', 'NP3HMOVL', 'NP3PRSPR', 'NP3PRSPL', 'NP3TTAPR', 'NP3TTAPL', 'NP3LGAGR', 'NP3LGAGL', 'NP3RISNG', 'NP3GAIT', 'NP3FRZGT', 'NP3PSTBL', 'NP3POSTR', 'NP3BRADY', 'NP3PTRMR', 'NP3PTRML', 'NP3KTRMR', 'NP3KTRML', 'NP3RTARU', 'NP3RTALU', 'NP3RTARL', 'NP3RTALL', 'NP3RTALJ', 'NP3RTCON'], two_option = 'None', decode_option = 'none2severe')
updrs_part3_df_copy['DBS_STATUS'].replace({0 : 'OFF', 1 : 'ON'}, inplace = True)
updrs_part3_df_copy = decode_columns(updrs_part3_df_copy, ['DYSKPRES','DYSKIRAT'], two_option = 'Uncertain', decode_option = 'yes/no')
...
updrs_cat = decode_columns(updrs_cat, ['NP1COG', 'NP1HALL', 'NP1DPRS', 'NP1ANXS', 'NP1APAT', 'NP1DDS'], two_option = 'None', decode_option = 'none2severe')
...
updrs_cat = decode_columns(updrs_cat, ['NP1SLPN', 'NP1SLPD', 'NP1PAIN', 'NP1URIN', 'NP1CNST', 'NP1LTHD', 'NP1FATG'], two_option = 'None', decode_option = 'none2severe')
...
updrs_cat = decode_columns(updrs_cat,['NP2SPCH','NP2SALV','NP2SWAL','NP2EAT','NP2DRES','NP2HYGN','NP2HWRT','NP2HOBB','NP2TURN','NP2TRMR','NP2RISE','NP2WALK','NP2FREZ'], two_option = 'None', decode_option = 'none2severe')
...
updrs_cat = decode_columns(updrs_cat, ['RMEXAM','DBSYN', 'OFFEXAM', 'OFFEXAM', 'ONEXAM'], two_option = 'Uncertain', decode_option = 'yes/no')
...
ppmi_merge = decode_columns(ppmi_merge,  ['African.Berber.Race','Ashkenazi.Jewish.Race', 'Basque.Race','Hispanic.Latino.Race', 'Asian.Race', 'African.American.Race', 'Hawian.Other.Pacific.Islander.Race', 'Indian.Alaska.Native.Race', 'Not.Specified.Race',  'White.Race'], two_option = np.NaN, decode_option = 'yes/no')
...
ppmi_merge['Event.ID.Date'] = ppmi_merge['Event.ID.Date'].apply(convert_date)
...
def extract_image_id(image_name):
    if isinstance(image_name, str):
        ...
        return image_id.split('-')[1] + '-' + image_id.split('-')[2] + '-' + image_id.split('-')[4]
    return ''
ppmi_merge['ImageID'] = ppmi_merge['T1.s3.Image.Name'].apply(extract_image_id)
