...
for event_id in event_id_list : 
    ...
    if event_id == 'Screening' : 
    ...
    elif event_id == 'Visit Month 36' : 
        #### Get subjects that are listed in month 24 and only get stats from these subej
        enroll_sporadic_pd = ppmi_merge[ppmi_merge[subtype_col]=='Sporadic']        
        baseline_only = enroll_sporadic_pd[enroll_sporadic_pd['Event.ID']==event_id]
        df3 = baseline_only[baseline_only['Subject.ID'].isin(mo24_n)] # get list of mo24 for year 3 follow up 
        baseline_only = df3[df3['Event.ID'] == 'Visit Month 36']
        ...
        if event_id == 'Visit Month 24' :
            mo24_n = baseline_only['Subject.ID'].to_list()
            mo24_n = set(mo24_n) # list of subject ids that have a baseline image 
