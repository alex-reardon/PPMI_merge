...
# If one event id has more than 1 LEDD entry - take max LEDD only FIXME
...
# Create a column with True/False for LEDD rows we want to keep (max) or only LEDD for that date 
subids = ppmi_merge_temp['PATNO'].unique() # Unique subject ids
eventids = ppmi_merge_temp['EVENT_ID'].unique() # Unique event ids
ppmi_merge_temp['LEDD_keep'] = False
for eventid in eventids : 
    for subid in subids :
        df = ppmi_merge_temp.loc[(ppmi_merge_temp['PATNO'] == subid) & (ppmi_merge_temp['EVENT_ID'] == eventid) & (ppmi_merge_temp['LEDD'].notna())]
        if len(df) == 1: 
            idx = df.index
            ppmi_merge_temp.loc[idx, 'LEDD_keep'] = True
        if len(df) > 1 : 
            df['LEDD'] = df['LEDD'].astype(float)
            maxidx = df.LEDD.idxmax()
            ppmi_merge_temp.loc[maxidx, 'LEDD_keep'] = True
        
LEDD_new_df = ppmi_merge_temp[ppmi_merge_temp['LEDD_keep'] == True]
LEDD_new_df.drop(['LEDD_keep'], axis = 1, inplace = True)
...
subids = ppmi_merge_temp['PATNO'].unique() # Unique subject ids
eventids = ppmi_merge_temp['EVENT_ID'].unique() # Unique event ids
ppmi_merge_temp['LEDD_keep.ongoing'] = False
for eventid in eventids : 
    for subid in subids :
        df = ppmi_merge_temp.loc[(ppmi_merge_temp['PATNO'] == subid) & (ppmi_merge_temp['EVENT_ID'] == eventid) & (ppmi_merge_temp['LEDD.ongoing'].notna())]
        if len(df) == 1: 
            idx = df.index
            ppmi_merge_temp.loc[idx, 'LEDD_keep.ongoing'] = True
        if len(df) > 1 : 
            df['LEDD.ongoing'] = df['LEDD.ongoing'].astype(float)
            maxidx = df['LEDD.ongoing'].idxmax()
            ppmi_merge_temp.loc[maxidx, 'LEDD_keep.ongoing'] = True
        
LEDD_new_df = ppmi_merge_temp[ppmi_merge_temp['LEDD_keep.ongoing'] == True] # Keep only portion of df where LEDD_keep.ongoing == True (max or only LEDD entry)
LEDD_new_df.drop(['LEDD_keep.ongoing'], axis = 1, inplace = True)
ppmi_merge = pd.merge(ppmi_merge, LEDD_new_df, on = ['PATNO','EVENT_ID'], how = "left") # Merge into ppmi_merge
