...
ppmi_merge = merge_new_csv(ppmi_merge, 'Demographics.csv', ['PATNO','EVENT_ID','SEX', 'HANDED', 'BIRTHDT','AFICBERB','ASHKJEW','BASQUE','HISPLAT','RAASIAN','RABLACK','RAHAWOPI','RAINDALS','RANOS','RAWHITE']) # Bday, sex, handedness, race
...
ppmi_merge.rename(columns = {'SEX' : 'Sex', 'HANDED' : 'Handed', 'BIRTHDT' : 'BirthDate', 'WGTKG' : 'Weight(kg)', 'HTCM' : 'Height(cm)', 'SXDT' : 'First.Symptom.Date', 'PDDXDT': 'PD.Diagnosis.Date', 'AFICBERB' : 'African.Berber.Race','ASHKJEW':'Ashkenazi.Jewish.Race', 'BASQUE' : 'Basque.Race', 'HISPLAT' : 'Hispanic.Latino.Race', 'RAASIAN' : 'Asian.Race', 'RABLACK' : 'African.American.Race', 'RAHAWOPI' : 'Hawian.Other.Pacific.Islander.Race', 'RAINDALS' : 'Indian.Alaska.Native.Race', 'RANOS' : 'Not.Specified.Race', 'RAWHITE': 'White.Race'}, inplace = True) # Rename columns
...
## LEDD Medication Status - without ongoing stop dates
...
LEDD_med_df['STOPDT3'].fillna('01/2022', inplace = True) # Fill in ongoing stopdate (12/2021 is max infodt in ppmi_merge file)
...
ppmi_merge_temp = ppmi_merge_temp[ppmi_merge_temp["LEDD.ongoing"].str.contains("LD") == False] # FIXME not sure what to do with LD x .33 etc rows - for now remove 
uids = ppmi_merge_temp['PATNO'].unique() # Unique subject ids
myevs = ppmi_merge_temp['EVENT_ID'].unique() # Unique event ids
ppmi_merge_temp['LEDD2.ongoing'] = False
for myev in myevs : 
    for u in uids :
        selu = ppmi_merge_temp.loc[(ppmi_merge_temp['PATNO'] == u) & (ppmi_merge_temp['EVENT_ID'] == myev) & (ppmi_merge_temp['LEDD.ongoing'].notna())]
        if len(selu) == 1: 
            idx = selu.index
            ppmi_merge_temp.loc[idx, 'LEDD2.ongoing'] = True
        if len(selu) > 1 : 
            selu['LEDD.ongoing'] = selu['LEDD.ongoing'].astype(float)
            maxidx = selu['LEDD.ongoing'].idxmax()
            ppmi_merge_temp.loc[maxidx, 'LEDD2.ongoing'] = True
        
ledd_keep = ppmi_merge_temp['LEDD2.ongoing'] == True
LEDD_new_df = ppmi_merge_temp[ledd_keep]
LEDD_new_df.drop(['LEDD2.ongoing'], axis = 1, inplace = True)
ppmi_merge = pd.merge(ppmi_merge, LEDD_new_df, on = ['PATNO','EVENT_ID'], how = "left") # Merge into ppmi_merge
...
for col_name in race_list :
    ppmi_merge[col_name].replace({0 :False , 1 : True, 2 : np.NaN}, inplace = True) 
