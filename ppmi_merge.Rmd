...
ppmi_merge = merge_new_csv(ppmi_merge,  'PD_Diagnosis_History.csv', ['PATNO', 'EVENT_ID', 'SXDT','PDDXDT'])
...
ppmi_merge = merge_new_csv(ppmi_merge, 'Conclusion_of_Study_Participation.csv', ['PATNO', 'EVENT_ID', 'COMPLT', 'WDRSN', 'WDDT']) # Visit date, weight and height 
ppmi_merge['COMPLT'].replace({1.0 : 'Yes', 0 : 'No'}, inplace = True) 
ppmi_merge['WDRSN'].replace({1 : 'Adverse Event', 2.0 : 'Completed study per protocol',3.0 : 'Death', 4.0 : 'Family, care-partner, or social issues', 5.0 : 'Lost to follow up', 6.0 : 'Non-compliance with study procedures' , 7.0 : 'Transportation/Travel issues' , 8.0 : 'Institutionalized' , 9.0 : 'Subject transitioning to a new cohort' , 10.0 : 'Subject withdrew consent', 11.0 : 'Investigator decision', 12.0 : 'Sponsor decision', 13.0 : 'Informant/caregiver decision' , 20.0 : 'Other'}, inplace = True) 
...
#### IMAGING INFO ####
## DATSCAN
datscan_analysis_df = pd.read_csv(ppmi_download_path + 'DaTScan_Analysis.csv') # Variables already decoded 
datscan_analysis_df.drop(['PROTOCOL','DATSCAN_OTHER_SPECIFY'], axis = 1, inplace = True)
ppmi_merge = pd.merge(ppmi_merge, datscan_analysis_df, on = ['PATNO','EVENT_ID'], how = "outer")

datscan_imaging_df = pd.read_csv(ppmi_download_path + 'DaTScan_Imaging.csv')
datscan_imaging_df.drop(['REC_ID','ORIG_ENTRY','LAST_UPDATE'], axis = 1, inplace = True)
datscan_imaging_df['DATSCAN'].replace({0 : 'Not Completed', 1: 'Completed', 2 : 'Completed using a previously acquired DaTscan (i.e., acquired prior to participant\'s consent to PPMI)' }, inplace = True)
datscan_imaging_df['SCNLOC'].replace({ 1 : 'Site', 2 : 'IND'}, inplace = True )
datscan_imaging_df['SCNINJCT'].replace({ 1 : 'DaTSCAN', 2 : 'Beta-CIT'}, inplace = True)
datscan_imaging_df['VSINTRPT'].replace({ 1.0 : 'Consistent with evidence' , 2.0 : 'Not consistent with evidence', 3.0 : 'No visual interpretation report provided'}, inplace = True)
datscan_imaging_df['VSRPTELG'].replace({1 : 'Eligible', 2 : 'Not eligible'}, inplace = True)
datscan_imaging_df.rename(columns = {'INFODT':'DaTScan.INFODT','SCNLOC' : 'Location.Scan.Completed' , 'PREVDATDT' : 'Date.DaTscan.Imaging.Completed.Previously', 'SCNINJCT' : 'Scan.Injection', 'VSINTRPT' : 'Visual.Interpretation.Report', 'VSRPTELG' : 'Visual.Interpretation.Report(eligible/not)'}, inplace = True)
ppmi_merge = pd.merge(ppmi_merge, datscan_imaging_df, on = ['PATNO','EVENT_ID'], how = "outer") # Merge

## If INFODT in ppmi_merge is NAN - fill in with datscan imagign_df date 
