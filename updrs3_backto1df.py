...
ppmi_merge_temp.to_csv('/Users/areardon/Desktop/ppmi_merge_temp.csv')
...
LEDD_max = ppmi_merge_temp.loc[ppmi_merge_temp.groupby(['PATNO', 'EVENT_ID'])['LEDD'].idxmax()].reset_index(drop=True) # Get max 
LEDD_max.rename(columns = {'LEDD' : 'LEDD.max'}, inplace = True) # Rename 
ppmi_merge = pd.merge(ppmi_merge, LEDD_max, on = ['PATNO','EVENT_ID'], how = "left") # Merge into ppmi_merge
...
#LEDD_med_df = pd.concat([func(row) for _, row in LEDD_med_df.iterrows()], ignore_index=True, axis=1).T
...
LEDD_max = ppmi_merge_temp.loc[ppmi_merge_temp.groupby(['PATNO', 'EVENT_ID'])['LEDD.ongoing'].idxmax()].reset_index(drop=True)
...
for row_num in range(len(ppmi_merge)) :
    if ppmi_merge['LD'].loc[row_num] == 'LD x 0.33' : 
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] = float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num]) * 0.33 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
    elif ppmi_merge['LD'].loc[row_num] == '(150.0 + LD) x 0.33' : 
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] = (150.0 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])) * 0.33 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
    elif ppmi_merge['LD'].loc[row_num] == '(600.0 + LD) x 0.33' :
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] =  (600.0 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])) * 0.33 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
    elif ppmi_merge['LD'].loc[row_num] == 'LD x 0.5':
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] = float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num]) * 0.5 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
    elif ppmi_merge['LD'].loc[row_num] =='(800.0 + LD) x 0.33':
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] = (800.0 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])) * 0.33 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
    elif ppmi_merge['LD'].loc[row_num] == '(300.0 + LD) x 0.33' :
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] = (300.0 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])) * 0.33 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
    elif ppmi_merge['LD'].loc[row_num] == '(225.0 + LD) x 0.33' : 
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] = (225.0 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])) * 0.33 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
    elif ppmi_merge['LD'].loc[row_num] == '(450.0 + LD) x 0.33' :
        ppmi_merge['LEDD.ongoing.sum'].loc[row_num] = (450.0 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])) * 0.33 + float(ppmi_merge['LEDD.ongoing.sum'].loc[row_num])
...
ppmi_merge = merge_new_csv(ppmi_merge, 'Modified_Semantic_Fluency.csv', ['PATNO', 'EVENT_ID', 'DVS_SFTANIM'], merge_on = ['PATNO', 'EVENT_ID'], merge_how = "outer")
...
fixed_var_list = ['Enroll.Diagnosis', 'Enroll.Subtype','Consensus.Diagnosis', 'Consensus.Subtype', 'Subject.Phenoconverted', 'BirthDate', 'Sex', 'Handed', 'Analytic.Cohort','African.Berber.Race','Ashkenazi.Jewish.Race', 'Basque.Race', 'Hispanic.Latino.Race', 'Asian.Race', 'African.American.Race', 'Hawian.Other.Pacific.Islander.Race', 'Indian.Alaska.Native.Race', 'Not.Specified.Race',  'White.Race'] # fixed variables
